...
$(".events-list__filter").addEventListener("change", chooseOptionChange, {
    passive: true,
});
...
function chooseOptionChange() {
    ...
    if (event.target.closest("#sort") || event.target.closest("#hi-low"))
        sortCardsArray();
...
function search() {
    ...
    events.forEach((item) => {
        if (item.theme.toLowerCase().includes(searchValue))
            evensArray.push(item);
        if (searchValue !== "") {
            if (item.day.toLowerCase().includes(searchValue))
                evensArray.push(item);
            if (item.month.toLowerCase().includes(searchValue))
                evensArray.push(item);
        }
    });
    sortCardsArray();
...
function pagesActive() {
    $$(".events-list__number")[cardsCounter - 1].classList.add(
        "events-list__number_active"
    );
}
function eventCard(obj) {
    eventList.insertAdjacentHTML(
        ...
        `<article id="${obj.id}" class="events-list__event event">
         ...
         <a href="./event-page.html" class="event__btn btn_1">View more</a>
     </article>`
    );
}
